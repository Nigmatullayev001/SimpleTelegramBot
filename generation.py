...
def generate_file(fmt, uid, items, settings=None, template_name=None):
    ...
    if fmt == "pdf":
        ...
        styles.add(ParagraphStyle(name="Caption", alignment=TA_CENTER, fontSize=int(text_size * 0.8),
                                  leading=text_size, textColor="gray"))
        ...
        if not story:
            story.append(Paragraph("Bo‘sh fayl", styles["Center"]))
        doc.build(story)
        return filename
        ...
        for item in items:
            if item[0] == "text":
                ...
                if para.runs:
                    para.runs[0].font.size = Pt(text_size)
                else:
